...
// Mirrors api/src/handlers/userContribution.ts - one person's work on one word, read-only.
// Keyed on the contribution rather than the word: that is what the user page links to, and
// a 'new_entry' proposal has no word at all.

/** One contribution as evidence: what they claimed, and whether it is what the record now
 * says. `resolvedValue` is a shared ContributionOutcome where one was resolved (0013) -
 * unknown here rather than typed, because a pre-0013 row has none and a 'new_entry'
 * proposal never had one. */
export interface UserContributionClaim {
  ...
  /** Null, not false, when it cannot be told - an undecided axis, or no fingerprint. */
...
export interface UserContributionRecording {
  ...
  segmentCount: number;
  releaseState: ReleaseState;
  /** Null for a take with no delivery copy - see 0022. Nothing to play in that case. */
...
export interface UserContributionDetail {
  ...
  email: string;
  displayName: string | null;
  /** The account's grant over its WRITTEN work. A recording carries its own, because a
   * voice and an account are granted separately. */
  releaseState: ReleaseState;
  contribution: UserContributionClaim;
  alsoOnThisWord: UserContributionClaim[];
  /** Null for a 'new_entry' proposal - there is no word yet. */
  word: {
    wordId: string;
    displayText: string;
    syllables: string[];
